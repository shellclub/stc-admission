    ...
    if ($_FILES['file_import']['size'] > 0) {
        ...
        while (($column = fgetcsv($file, 10000, ",")) !== FALSE) {
           ...
           // $idcard = mysqli_real_escape_string($conn, $column[0]);
            ...
            $major  = mysqli_real_escape_string($conn, $column[2]);
            $de_id  = mysqli_real_escape_string($conn, $column[3]);
            $level  = (int)$column[4];
            $score  = (int)$column[5];

            // ตรวจสอบว่ามี idcard นี้อยู่ในระบบหรือยัง (เพื่อไม่ให้ Insert ซ้ำ)
            $check = $conn->query("SELECT id FROM tb_user_new WHERE id = '$id'");
            
            if($check->num_rows == 0) {
                // คำสั่ง Insert
                $sql = "INSERT INTO tb_user_new ( id, name, major, de_id, level, score) 
                        VALUES ( '$id', '$name', '$major', '$de_id', '$level', '$score')";
