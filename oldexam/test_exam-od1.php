<?php 
 ...
 // --- ระบบแบ่งหน้า ---
$limit = 10; // แสดงหน้าละ 10 ข้อ
$page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$start_index = ($page - 1) * $limit;

// --- ฟังก์ชันเช็คคำตอบเดิมที่เคยเลือกไว้ ---
function getSavedChoice($std_id, $q_id, $choice_no, $conn) {
    // เช็คก่อนว่า Connection ยังเปิดอยู่ไหม
    if (!$conn || $conn->connect_error) {
        return ""; 
    }

    if (empty($std_id) || empty($q_id)) return "";
    
    $s_id = mysqli_real_escape_string($conn, $std_id);
    $question_id = mysqli_real_escape_string($conn, $q_id);
    
    $sql = "SELECT selected_choice FROM tb_user_answers WHERE std_id='$s_id' AND question_id='$question_id' LIMIT 1";
    $res = $conn->query($sql);
    
    if ($res && $res->num_rows > 0) {
        $row = $res->fetch_assoc();
        return ($row['selected_choice'] == $choice_no) ? "checked" : "";
    }
    return "";
}
...
/* UI สำหรับตัวเลือกข้อสอบบนมือถือ */
.choice-container { display: block; position: relative; margin-bottom: 12px; cursor: pointer; }
.choice-container input { position: absolute; opacity: 0; }
.checkmark { 
    display: flex; align-items: center; padding: 15px; 
    background-color: #f8f9fa; border: 2px solid #ddd; border-radius: 10px; transition: 0.2s; 
}
.choice-container input:checked ~ .checkmark { 
    background-color: #d4edda; border-color: #28a745; color: #155724; box-shadow: 0 4px 6px rgba(0,0,0,0.1); 
}
.choice-img { max-width: 100%; height: auto; margin-bottom: 10px; border-radius: 5px; }
.choice-text { font-size: 18px; margin: 0; font-weight: 500; }
/* ปรับแต่งปุ่มเลขหน้า */
.pagination > .active > a { background-color: #28a745; border-color: #28a745; }
  </style>
</head>
...
<div id="section1" class="container-fluid">
                            ...
                            <!-- /.box-header -->
                                    <div class="box-body">
                                           ...
                                           <p class="text-primary">ผู้สมัคร: ชื่อ  <?php echo $sname;  ?>  </p>
                                            ...
                                            <p class="text-primary">ระดับชั้น  <?php echo $data_level;  ?>   </p>
                                     </div>
                                <!-- /.box-body -->
                            </div>
                                    <!-- /.box -->
 ...
 <div id="section_exam" class="container-fluid" style="padding-top:20px;">
    <?php 
    // วนลูปแสดงข้อสอบเฉพาะ 10 ข้อของหน้านั้นๆ
    for($i=$start_index; $i < ($start_index + $limit) && $i < 60; $i++){    
    ?>
    <div class="box box-danger">
        <div class="box-header with-border">
            <h3 class="text-primary">ข้อที่ <?php echo $i+1; ?>:</h3>
            <?php if($img_q[$i]!=''){ ?>
                <img src="ImagesQuestions/<?php echo $img_q[$i]; ?>" class="img-responsive" style="max-height:250px;"/>
            <?php } ?>
        ...
        <div class="box-body">
            <div class="row">
                <?php for($n=1; $n<=4; $n++) { 
                    $c_text = ${"choice".$n}[$i];
                    $c_img = ${"choice".$n."_img"}[$i];
                ?>
                <div class="col-xs-12 col-sm-6">
                    <label class="choice-container">
                        <input type="radio" 
                               name="select_<?php echo $id_s[$i]; ?>" 
                               value="<?php echo $n; ?>"
                               <?php echo getSavedChoice($ids1, $id_s[$i], $n, $conn); ?>
                               onchange="saveToDB('<?php echo $ids1; ?>', '<?php echo $id_s[$i]; ?>', <?php echo $n; ?>, <?php echo $i+1; ?>)">
                        <span class="checkmark">
                            <div style="width: 100%;">
                                <?php if($c_img != '') { ?>
                                    <img src="ImagesQuestions/<?php echo $c_img; ?>" class="choice-img" />
                                <?php } ?>
                                <p class="choice-text"><strong><?php echo $n; ?>.</strong> <?php echo $c_text; ?></p>
                            </div>
                        </span>
                    </label>
                </div>
        ...
        <ul class="pagination pagination-lg">
            <?php for($p=1; $p<=6; $p++) { ?>
                <li class="<?php echo ($page == $p) ? 'active' : ''; ?>">
                    <a href="?std_id=<?php echo $ids1; ?>&page=<?php echo $p; ?>"><?php echo $p; ?></a>
                </li>
            <?php } ?>
        </ul>
    </div>
...
var num = new Array();
num[6]=null;
totalExamArray[61] = null;
var missedExam = "";
var tx_score="";
var numt=0;
for(var a=1;a<61;a++) totalExamArray[a]=0;
var data;
 var chk=0;
 var hour  =0;
 var minute =0;
...
function radio_check(examDone) {
        ...
        if(totalExam==60){
            ...
            document.getElementById("numtest").innerHTML = "คุณทำข้อสอบครบ 60 ข้อ";
            tx_scroe ="ทำข้อสอบครบ 60 ข้อ";
        }
        else {
...
function ping(){
       ...
       var answer = confirm("คุณทำข้อสอบไป "+numt+" ข้อ ยังไม่ครบ 60 ข้อ ต้องการส่งคำตอบ กด ปุ่ม OK ถ้าต้องการ ยกเลิกกดปุ่ม Cancel  ข้อสอบที่ยังไม่ได้ทำคือ \n "+missedExam+" \n ต้องการส่งคำตอบ กด ปุ่ม OK ถ้าต้องการ ยกเลิกกดปุ่ม Cancel");
        missedExam ="";
        if (answer) {
...
function alertS(){
  const swalWithBootstrapButtons = Swal.mixin({
        ...
        swalWithBootstrapButtons.fire({
              ...
              text: "คุณทำข้อสอบไป "+numt+" ข้อ จาก 60 ข้อ  ต้องการส่งคำตอบ กด ตกลงส่งคำตอบ  ถ้าต้องการยกเลิกกดปุ่ม กลับไปทำข้อสอบ  ข้อสอบที่ยังไม่ได้ทำคือ ข้อ : \n "+missedExam+" \n",
              icon: "warning",
      ...
      xmlhttp.onreadystatechange = function() {
        ...
        //ping();       if()
                        if(minute > 59) document.form1.submit();
                            ...
                            if(numt==60) document.form1.submit() ; 
                            else {
                              alertS();
                            }
...
function countTimer() {
   ...
   if(hour >= 1) {
    ...
    if(numt==60) 
        document.getElementById("myBtn").innerHTML = "ทำข้อสอบครบ 60 ข้อ";
    else{
        if(chk==1) {
 ...
 function saveToDB(stdId, qId, choiceValue, examNumber) {
    // 1. เรียกฟังก์ชันนับจำนวนข้อเดิมของคุณ (ถ้ามี)
    radio_check(examNumber);

    // 2. ส่ง AJAX ไปบันทึกที่ Server ทันที
    $.ajax({
        url: 'save_answer.php',
        type: 'POST',
        data: {
        ...
        success: function(res) {
            console.log("Auto-saved Question ID: " + qId);
        },
        error: function() {
            alert("การเชื่อมต่อมีปัญหา! คำตอบข้อนี้อาจไม่ถูกบันทึก กรุณาตรวจสอบอินเทอร์เน็ต");
        }
    });
}
